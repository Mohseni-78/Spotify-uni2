...
const Body = ({ chooseTrack }) => {
  ...
  const filterMusic = newReleases.filter((item) =>
    item.title.toUpperCase().includes(search.toUpperCase())
  );

  return (
    <section className="bg-black ml-24 py-4 space-y-8 md:max-w-[1000px] flex-grow md:mr-2.5">
      ...
      <div className="flex gap-x-8 absolute min-w-full md:relative ml-6">
        ...
        <div className="w-full pr-11">
          <h2 className="text-white font-bold mb-3">
            {!search ? "New Releases" : "Tracks"}
          </h2>
          <div
            className="space-y-3 border-2 border-[#262626] rounded-2xl p-3 bg-[#0D0D0D] overflow-hidden 
          h-[400px] md:h-96 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-thumb-rounded hover:scrollbar-thumb-gray-500 w-[700px]"
          >
            {!search
              ? newReleases
                  .slice(0, 5)
